import 'package:coffe_app_ui/models/coffee.dart';
import 'package:coffe_app_ui/screens/home_scree.dart';
import 'package:flutter/material.dart';

class Models {
  static List<Map<String, dynamic>> menuItems = [
    {'icon': Icons.home, 'destination': const HomeScreen(), 'name': 'Home'},
    {
      'icon': Icons.heart_broken_outlined,
      'destination': const Center(child: Text('favourites')),
      'name': 'favourites',
    },
    {
      'icon': Icons.shopping_bag,
      'destination': const Center(child: Text('favourites')),
      'name': 'cart',
    },
    {
      'icon': Icons.notification_add,
      'destination': const Center(child: Text('favourites')),
      'name': 'notificaitons',
    },
  ];
  static List<String> coffeeCategories = [
    'All Coffee',
    'Machiato',
    'Late',
    'Americano',
    'All Coffee',
    'Machiato',
    'Late',
    'Americano',
    'Cappuccino',
    'Cappuccino',
  ];

  static List<String> gridCoffeImages = [
    'data:image/jpeg;base64,',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
    'assets/images/woman-5899643.png',
  ];

  static List<Coffee> coffees = [
    Coffee(
      name: 'machiato and some thing else you bitch ',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, ther a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, there a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, there a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, theres a wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, there wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, therea wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
    Coffee(
      name: 'machiato',
      price: 89,
      image: 'image',
      rating: 4.4,
      type: 'summerO',
      slung: 'slung',
      description:
          'Coffee is a beverage brewed from roasted, ground coffee beans, known for its stimulating effect due to caffeine. The most common types of coffee beans are Arabica and Robusta. Arabica beans are prized for their complex, often fruity and floral flavors, while Robusta beans are known for their higher caffeine content and bolder, earthy taste. Beyond the beans, there wide array of coffee drinks, from simple black coffee and espresso to lattes, cappuccinos, and cold brew variations. ',
    ),
  ];
}
